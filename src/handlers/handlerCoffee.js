...
export const getTypesOfCoffee = async (req, res) => {
  try {
...
export const getRoastProfileOfCoffee = async (req, res) => {
  try {
    let roasts = await getAllRoasts();
    res.status(200).json(roasts);
  } catch (error) {
    res.status(404).json({ error: error.message });
  }
};
export const getOriginsOfCoffee = async (req, res) => {
  try {
